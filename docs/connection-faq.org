...
Caveats: resume works cleanly only if the working directory still
exists.  With local-disk tiering (=slurm.local_disk= on a confined
target) the working clone under =/local/job<ID>/= is gone with the job,
but committed work is already on the shared mirror and uncommitted work
is restored from the WIP snapshot on the next launch; the transcript
key, however, is the clone's path, which changes with the job id, so
=--resume= needs the old key's directory.  On an unconfined target a
mirror on =/local= is gone with the node.  An in-flight background *workflow*
