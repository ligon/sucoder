...
suspend and resume frequently, a system suspend/resume hook
(=systemd-sleep= on Linux, =sleepwatcher= on macOS) can run =tunnel down=
on resume to clear the zombies immediately --- though it can't re-auth for
you (that OTP is yours to type).
