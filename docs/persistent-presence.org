...
1. *Agent-exit teardown.*  With =; exec bash -l= the tmux session
   ...
   /Resolved: (a) only./  =exec bash -l= shipped and stays --- it buys a
   window for manual repo checks after the agent exits.  (c) was never
   implemented, so a confined job really does hold its slice for the full
   =--time= after a clean =/exit=.  =sucoder sessions= makes that visible
   (=! agent exited=) rather than fixing it; reconsider (c) once the
   surrounding process is bulletproof.  The cost scales with the target:
   on a 12-day =carleton-htc= slice it is 12 days.
...
To validate the memory cap, do *not* rely on
=cat /sys/fs/cgroup/memory.max=.  Slurm here gives the job no cgroup
namespace, so =/proc/self/cgroup= shows the full host path
(=/system.slice/.../job_<id>/...=) and =/sys/fs/cgroup/= is the host root
cgroup, whose =memory.max= is =max= (or unreadable).  Instead derive the
leaf from =/proc/self/cgroup= and walk upward to the first enforced
(non-=max=) =memory.max=.  This also works when a namespace is present
(then the leaf is =/= and the root is read directly):
#+begin_src shell
...
- *No wall-clock cap.*  =MaxWall= is empty; the partition =MaxTime= is
  ...
  =savio_normal= caps at 72h.)  *Confirmed 2026-06-29:* the partition's
  =DefaultTime=NONE=, so Slurm falls back to =MaxTime=UNLIMITED= -- even
  *omitting* =--time= yields an unlimited job, so there is no silent
  default cap to fear.  The =3-00:00:00= we set is therefore pure courtesy
  cadence, not a necessity; this is also why the "self-renewing job"
  idea was dropped (it pre-empts a walltime that is never enforced, and
  an in-job renewer dies with its node at the only real ceiling -- a
  maintenance reboot, which only an *external* watcher can recover from).
...
targets:
  carleton-htc:
    ...
    slurm:
      ...
      local_disk: true             # working clone + caches on /local; every commit
                                   # lands on the shared mirror (docs/local-disk-tiering.org)
...
** Handoff / rehydration
- Before turnover the agent writes a structured note to
  =.sucoder/handoff.org= in its working tree and commits it: current
  task, branch, last commit SHA, open questions, next action.  Under
  local-disk tiering the commit is what carries it to the shared mirror
  (writing into =~/mirrors/<mirror>= by hand would block every publish).
