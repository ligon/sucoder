...
- Create or identify the human login and the agent login (defaults: human = =ligon=, agent = =coder=, group = =coder=).
...
- Copy =config.example.yaml= (or =default_config.yaml=) to =~/.sucoder/config.yaml= and adjust:
  ...
  - per-mirror entries: canonical repo path, mirror name, base branch, branch prefixes, agent launcher flags (for Codex/nvm)
...
- Node via nvm if you want to pin a Codex runtime; configure =agent_launcher.nvm= in the mirror config when used.
- Codex CLI installed and callable for the human; =sucoder= injects =--yolo= automatically when launching agents.
