...
No config file needed.  =sucoder= detects the git repo, mirrors it under =/var/tmp/coder-mirrors/=, and launches the agent.  Add =--task fix-login= to start on a dedicated branch.
