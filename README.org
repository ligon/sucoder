...
- =cert_file= --- path to a local SSH *certificate* private key, presented on
  ...
  =scripts/brc-connect.sh= (the BRC/Savio MSM CA caps a cert at 12h).  It is
  applied only to the gateway --- the login/DTN hops keep their publickey
  auth through the mux.  A missing or expired cert degrades cleanly to the
  interactive prompt; =sucoder -T <target> tunnel doctor= reports the cert's
  validity.
...
pattern: =up= warms the tunnels, starts a web app on the target's
compute node (if it isn't already running), forwards its port, and
opens the URL in the local browser; =url= re-fetches a fresh URL from
the *running* app, repairs the forward, and reopens it.  Both modes are
idempotent; see the header comment for the =TARGET= / =NODE= /
=APP_BIN= / =OPENER= overrides.
