...
- Python >= 3.9 with pip
- git
- git-crypt (optional; only needed if you have other =.gitattributes=-encrypted files. =.mcp.json= no longer contains secrets --- see "MCP servers" below for the new token-handling guidance)
...
2. *Pin a login node* --- SSH to the gateway, run =hostname=, store
   ...
   Subsequent connections jump to the same node via =-J=.
...
#+begin_src shell
sucoder -T savio collaborate --node n0047.savio2
sucoder -T savio attach --node n0047.savio2
#+end_src
...
The =github= server requires a =GITHUB_TOKEN= environment variable.  *Do not commit the token to =.mcp.json=.* Earlier revisions of this repository did so (encrypted via =git-crypt=), but a security audit flagged that approach: the decrypted plaintext is visible to anyone with read access to a working tree, and any token committed to history --- even encrypted --- is permanently compromised once a single key is leaked.  The current =.mcp.json= no longer carries an =env= block; the GitHub MCP server reads =GITHUB_TOKEN= from its parent process's environment.

Set the token in the agent user's shell environment so it propagates to spawned MCP servers.  The simplest approach is to add it to the agent user's =.bashrc= (or equivalent), because sucoder sources the agent's profile when wrapping commands:

#+begin_src shell
# As root or via sudo, append to /home/coder/.bashrc (replace USER and TOKEN):
echo 'export GITHUB_TOKEN="ghp_YOUR_REAL_TOKEN_HERE"' \
    | sudo tee -a /home/coder/.bashrc
sudo chown coder:coder /home/coder/.bashrc
sudo chmod 0640 /home/coder/.bashrc
#+end_src

If you prefer to keep the token out of any on-disk file altogether, configure your launcher to inject it via =sudo --preserve-env=GITHUB_TOKEN= or an equivalent mechanism.  Either way, no GitHub credential should appear in =.mcp.json= or anywhere else under git tracking.

After provisioning the new mechanism, *rotate* any token that ever appeared in committed history --- the previous repo HEAD shipped a literal PAT, so that token must be considered burned.

*** Note on =git-crypt=

=git-crypt= is still configured for =.mcp.json= via =.gitattributes=, but the file no longer contains secrets, so the encryption is now redundant.  You can safely:

  - leave it as-is (no harm; future secret-bearing files in =.mcp.json= would still be encrypted), or
  - remove the =filter=git-crypt diff=git-crypt= line for =.mcp.json= from =.gitattributes= and re-commit the file as plain text.

#+begin_src shell
# Install git-crypt if needed for other encrypted files in this repo:
brew install git-crypt   # macOS
sudo apt install git-crypt  # Debian/Ubuntu

# Unlock encrypted files (one-time per checkout):
git-crypt unlock
#+end_src

Collaborators who have been added via =git-crypt add-gpg-user= can unlock with their GPG key.
