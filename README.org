...
| =audit=              | Run compliance review of agent-written skills |
...
- audit :: Run a compliance review of agent-written skills (see [[*Skills Auditing][Skills Auditing]]).
...
A dedicated compliance agent reviews skill changes made by the working
agent.  The auditor runs as a separate Unix user (=auditor=) that is
intentionally /not/ in the =coder= group, so write access to agent
files is prevented by Unix permissions rather than prompt instructions.
Read access is granted via world-readable bits (=o+r=).
...
1. *Permissions check* --- Before examining content, the auditor verifies
   that all files under =~coder/.claude/skills/= are world-readable.
   Files the working agent has made unreadable (=chmod o-r=) are flagged
   immediately as suspicious.

2. *Audit mode* --- If no baseline exists (=refs/audited= ref), a full
   review runs.  Otherwise, only the diff since the last approved commit
   is examined.

3. *Auditor invocation* --- =claude -p= is run as the =auditor= user with
   a focused system prompt (=~/.sucoder/auditor_prompt.org=).  The auditor
   is instructed to detect instruction injection, scope creep,
