...
binary is chosen by a plain PATH lookup.  The launch does use =bash -lc=, but the
stock =~/.bashrc= returns early when non-interactive, so version managers wired up
there---nvm and friends---never load.  A shell you type =codex= into and a shell
=sucoder= launches can therefore resolve to different installs, and the stale one
wins silently.

Before each local launch, =sucoder= logs the absolute path and =--version= of the
binary it resolved:
...
comes earlier on PATH.  Note that =sudo= consults its own =secure_path= rather than
the caller's PATH, so a symlink under the agent user's home will not help a launch
that escalates; use a system directory such as =/usr/local/bin=.
