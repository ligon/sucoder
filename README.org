...
- the deadline watchdog snapshots uncommitted work to
  =refs/sucoder/wip/<name>= on the shared mirror every
...
INFO Tool versions on n0043.savio4:
  gh: gh version 2.67.0 (2025-02-11), floor 2.83.0 -- BELOW FLOOR [/global/home/users/u/bin/gh]
...
WARNING Tool preflight: gh: gh version 2.67.0 (2025-02-11), floor 2.83.0 -- BELOW FLOOR [...]
...
*Why this exists.*  A target's =gh= was 2.67.0 (February 2025).  Against a
server-side change fixed upstream in October 2025 ([[https://github.com/cli/cli/issues/11983][cli/cli#11983]]), every =gh pr
...
tool_preflight:
  ...
  floors:
    gh: "2.83.0"            # override a shipped floor
...
merged over the shipped defaults (=gh 2.83.0=, =git 2.34.0=, =jq 1.6=, =rg
13.0.0=, =tmux 3.0=), so naming one tool does not quietly stop checking the rest.
Only the =gh= floor is incident-derived---v2.83.0 is the first release published
after cli/cli#11983 was closed; the other four are conservative defaults, not
findings.  A floor =sucoder= cannot parse is rejected when the config loads, not
at launch.
