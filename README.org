...
Each worktree gets its own checkout while sharing the same object store
(no duplicate clone).  The human can monitor all worktrees from upstream:
...
** Remote Execution
Mirrors can also live on a remote host (e.g., an HPC cluster).  Define
a named target in the config:
...
Then use =--target= (=-T=) to select it:

#+begin_src shell
sucoder collaborate -a claude -T savio     # launch on cluster
sucoder worktrees -T savio --watch 30      # monitor from local
sucoder attach -T savio                    # reconnect via tmux
#+end_src

=sucoder= manages SSH ControlMaster connections (one authentication prompt,
all subsequent commands multiplex through the socket) and a port-forward
tunnel through the data transfer node for git push/pull.  The agent CLI
runs on the remote host; the canonical repo stays local.
