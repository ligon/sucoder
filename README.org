...
slice (cancels the job, kills its tmux, removes the forward).  =up= /
=url= are idempotent; see the header comment for the =TARGET= / =NODE= /
=APP_BIN= / =OPENER= / =WAIT_SECS= and =CS_*= slice overrides.
