...
=slurm.local_disk= (or =--local-disk= on the command line) moves the
...
For a =confined= target this is /tiering/ (design and measurements in
[[file:docs/local-disk-tiering.org][docs/local-disk-tiering.org]]):

- the batch job clones =~/mirrors/<name>= to
...
=wip_snapshot_minutes=.

Unconfined (=salloc=) targets still use the older layout, where the
whole mirror lives at =/local/mirrors= on one node and is only
reachable from that node (=--node= returns to it; a lost node orphans
unpulled work).  They move to tiering in a later release.
