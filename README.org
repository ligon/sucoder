...
6. Warm up the shared skills catalog so launches confirm access before work starts.
   ...
   codex read ~/.sucoder/skills/SKILLS.md
   #+end_src
   The listings should succeed without permission errors. The CLI helper highlights any unreadable
   paths, =sucoder mirrors-list= prints the configured mirrors (with their canonical and mirror directories),
   and the =codex read= command preloads the catalog into the session buffer.
...
- The CLI automatically appends `--yolo` when launching Codex so the sandbox allows writes to the mirror.  When invoking Codex manually, include `--yolo` (or an equivalent workspace-write mode) to avoid read-only failures.  The helper will also warn if it detects a read-only sandbox and still attempts a launch.

- Org authoring playground (documentation example)
  #+begin_src shell
  codex read docs/examples/org-authoring-demo.org
  #+end_src
...
1. Fetch the agent branch back into the canonical repository.
   #+begin_src shell
   git -C /home/ligon/src/project.git fetch /var/tmp/coder-mirrors/project coder/add-metrics-20251107164028
   #+end_src
2. Check out and review the branch inside the canonical repository.
   #+begin_src shell
   git -C /home/ligon/src/project.git checkout -b review/add-metrics FETCH_HEAD
   #+end_src
3. Fast-forward the reviewed branch to keep history linear (default):
   #+begin_src shell
   git -C /home/ligon/src/project.git checkout ligon/main
   git -C /home/ligon/src/project.git merge --ff-only review/add-metrics
   #+end_src
   ...
   #+begin_src shell
   git -C /home/ligon/src/project.git pull --ff-only /var/tmp/coder-mirrors/project coder/add-metrics-20251107164028
   #+end_src
...
log_dir: /home/ligon/.sucoder/logs
skills:
  - /home/ligon/.sucoder/skills  # optional global default for all mirrors (overridable per-mirror)
system_prompt: /home/ligon/.sucoder/system_prompt.org
mirrors:
  project:
    canonical_repo: /home/ligon/src/project.git
    mirror_name: project
    ...
    skills:
      - /home/ligon/.sucoder/skills/org-style
      - /home/ligon/.sucoder/skills/document-skill
    agent_launcher:
...
| system_prompt  | Inject system prompt        | (none)                                                | --system-prompt              | (none)                             | If unset, sucoder appends the prompt as trailing text when inline prompts are supported. |
...
** Pin Codex Node Version with nvm
Codex currently bundles Node.js 18, but =sucoder= can wrap the launch so that Codex
runs under a newer nvm-managed runtime (for example Node 22).
...
2. Update the mirror configuration so launches always source nvm before running Codex:
   #+begin_src yaml
   mirrors:
     project:
       canonical_repo: /home/ligon/src/project.git
...
- =exec codex …= so the rest of the CLI arguments (including the injected =--yolo= and
...
- To install a curated skill manually, download =skills/.curated/<name>= from https://github.com/openai/skills and place it under your skills directory (e.g., =~/.codex/skills= or whatever your agent uses).
...
- The shared catalog (`skills/SKILLS.md`) lists every bundled skill so agents discover them
  automatically.
...
- The upstream workflow is available in `skills/skill-creator/`; use it alongside `document-skill` when you need initialization or packaging scripts.
...
- =references/= — documentation to load on demand (each file includes a suggested `codex read` command).
...
- Add workflows for human operators to fetch agent branches back into their canonical repository.
