...
stale session records (job gone; 'sucoder release' clears):
...
partition/account/qos.  The listing is enumerated from =squeue=, not
from the session records under =~/.sucoder/sessions/=: those hold one
...
- the deadline watchdog snapshots uncommitted work to
  ...
  branch tip has not moved and the job that took it has ended;
