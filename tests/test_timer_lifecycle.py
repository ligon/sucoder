...
def test_timer_survives_starter_and_reuses_owner(timers, confined):
    ...
    owner = next(timers.root.glob(".cache/sucoder/timers/*/*/owner"))
...
def test_concurrent_starters_create_one_timer(timers):
    ...
    assert len(list(timers.root.glob(".cache/sucoder/timers/*/*/owner"))) == 1
...
def test_target_and_allocation_have_distinct_timers(timers):
    ...
    owners = list(timers.root.glob(".cache/sucoder/timers/*/*/owner"))
...
def test_dead_owner_metadata_does_not_signal_unrelated_process(timers):
    ...
    state = timers.root / ".cache/sucoder/timers" / timer_identity("example", "savio") / f"{node}-12"
...
def test_watchdog_warns_and_snapshots_without_changing_index(timers):
    ...
    warning = next(timers.root.glob(".cache/sucoder/timers/*/*/slurm-deadline-*.warn"))
